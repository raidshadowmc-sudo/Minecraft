...
async def create_ascend_image(player_data, ascend_data):
    ...
    if not PIL_AVAILABLE:
        print("PIL недоступен - изображение не может быть создано")
        return None
            ...
            if players and players.get('players'):
                ...
                if low_karma_players:
                    ...
                    for guild in bot.guilds:
                        ...
                        if guild.member_count and guild.member_count > 1000: # Assuming a large server is the main one
                            general = discord.utils.get(guild.text_channels, name='general') or discord.utils.get(guild.text_channels, name='main') or discord.utils.get(guild.text_channels, name='chat') or (guild.text_channels[0] if guild.text_channels else None)
            ...
            if visual:
                if PIL_AVAILABLE:
                    ...
                    if img_bytes:
                        ...
                        embed.add_field(name="⚠️ Изображение", value="Не удалось создать изображение", inline=False)
                else:
                    embed.add_field(name="⚠️ Изображение", value="Функция создания изображений недоступна (нет Pillow)", inline=False)
...
async def submit_quest(interaction: discord.Interaction, quest_name: str, attachment: discord.Attachment | None = None):
        try:
            await interaction.response.defer()
        ...
        async def on_submit(self, interaction: discord.Interaction):
            try:
                ...
                if self.color_input.value:
                    ...
                    try:
                        ...
                        # Именованные цвета
                            ...
                            color_value = 0x9b59b6
                        elif color_str.lower() in ['orange', 'оранжевый']:
                            color_value = 0xff9800
                        else:
                            # Попытка парсинга как обычное число
                            color_value = int(color_str, 16)
                    except ValueError:
                        color_value = 0x3498db  # Fallback
                ...
                if self.image_url_input.value:
                    if self.image_url_input.value.startswith('http'):
                        embed.set_image(url=self.image_url_input.value)
                
                # Создание view с дополнительными опциями
                view = EmbedActionView(embed, interaction.user.id)
                
                await interaction.response.send_message(
                    "✨ Ваше сообщение создано! Выберите действие:",
                    embed=embed,
                    view=view
                )
    ...
    class EmbedActionView(discord.ui.View):
        def __init__(self, embed, creator_id):
            super().__init__(timeout=300)
            self.embed = embed
            self.creator_id = creator_id
        
        @discord.ui.button(label="📤 Отправить в этот канал", style=discord.ButtonStyle.primary, emoji="📤")
        async def send_to_channel(self, interaction: discord.Interaction, button: discord.ui.Button):
            if interaction.user.id != self.creator_id:
                await interaction.response.send_message("❌ Только создатель может отправить это сообщение", ephemeral=True)
                return
            
            await interaction.response.send_message(embed=self.embed)
            await interaction.edit_original_response(content="✅ Сообщение отправлено!", embed=None, view=None)
        
        @discord.ui.button(label="📝 Редактировать", style=discord.ButtonStyle.secondary, emoji="📝")
        async def edit_embed(self, interaction: discord.Interaction, button: discord.ui.Button):
            if interaction.user.id != self.creator_id:
                await interaction.response.send_message("❌ Только создатель может редактировать", ephemeral=True)
                return
            
            # Новое модальное окно с текущими значениями
            modal = EmbedBuilderModal()
            # Предзаполнение значений
            modal.title_input.default = self.embed.title or ""
            modal.description_input.default = self.embed.description or ""
            modal.footer_input.default = self.embed.footer.text if self.embed.footer else ""
            
            await interaction.response.send_modal(modal)
        
        @discord.ui.button(label="🗑️ Удалить", style=discord.ButtonStyle.danger, emoji="🗑️")
        async def delete_embed(self, interaction: discord.Interaction, button: discord.ui.Button):
            if interaction.user.id != self.creator_id:
                await interaction.response.send_message("❌ Только создатель может удалить", ephemeral=True)
                return
            
            await interaction.response.edit_message(content="🗑️ Сообщение удалено", embed=None, view=None)
...
async def buy_command(interaction: discord.Interaction, item_name: str, player_nickname: str | None = None):
...
@bot.tree.command(name="apply", description="Применить купленный предмет (тему, градиент, роль, титул)")
async def apply_item_command(interaction: discord.Interaction, item_type: str, item_name: str, player_nickname: str | None = None):
    """Применение купленных предметов"""
    try:
        await interaction.response.defer()
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if not player_data or not player_data.get('players'):
                ...
                # Применить предмет
                apply_response = await session.post(
                    f"{WEBSITE_URL}/api/player/{player_id}/apply-item",
                    json={
                        'item_type': item_type,
                        'item_name': item_name,
                        'discord_user_id': str(interaction.user.id)
                ...
                if apply_response.status != 200:
                    error_data = await apply_response.json()
                    await interaction.followup.send(f"❌ {error_data.get('message', 'Ошибка применения предмета')}", ephemeral=True)
                    return
            ...
            embed = discord.Embed(
                ...
                description=f"**{player['nickname']}** применил **{item_name}** ({item_type})",
                color=0x9b59b6,
                timestamp=datetime.utcnow()
            )
            
            embed.add_field(name="🎨 Тип", value=item_type.title(), inline=True)
            embed.add_field(name="📦 Предмет", value=item_name, inline=True)
            embed.add_field(name="💡 Результат", value=result.get('message', 'Предмет успешно применен'), inline=False)
            
            embed.set_thumbnail(url=f"https://mc-heads.net/avatar/{player['nickname']}/100")
            embed.set_footer(text="Изменения видны на сайте")
            
            await interaction.followup.send(embed=embed)
            
        except Exception as e:
            print(f"Ошибка в команде apply: {e}")
            await interaction.followup.send("❌ Произошла ошибка при применении предмета", ephemeral=True)
